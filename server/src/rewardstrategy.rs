...
pub trait RewardStrategyGuesser: Send + Sync {
    fn reward_points_to_guesser(&self, player: &mut Player, number_of_guessers: usize, players_already_guessed: usize);
}
pub trait RewardStrategyDrawer: Send + Sync {
    fn reward_points_to_drawer(&self, player: &mut Player, number_of_guessers: usize, players_already_guessed: usize);
}
...
impl RewardStrategyGuesser for EqualRewardStrategy {
    fn reward_points_to_guesser(&self, player: &mut Player, _number_of_guessers: usize, _players_already_guessed: usize) {
        player.score += self.full_reward;
    }
}

impl RewardStrategyDrawer for EqualRewardStrategy {
    fn reward_points_to_drawer(&self, player: &mut Player, number_of_guessers: usize, _players_already_guessed: usize) {
...
impl RewardStrategyGuesser for LinearlyDecreasingRewardStrategy {
    fn reward_points_to_guesser(&self, player: &mut Player, number_of_guessers: usize, players_already_guessed: usize) {
        ...
        player.score += self.full_reward -
            points_for_last_guesser * (players_already_guessed - 1) as i64;
    }
}

impl RewardStrategyGuesser for ExponentiallyDecreasingRewardStrategy {
    fn reward_points_to_guesser(&self, player: &mut Player, _number_of_guessers: usize, players_already_guessed: usize) {
        player.score += (self.full_reward as f64 * ((1.0 - self.decrease_per_position).powi(
            players_already_guessed as i32,
        )) as f64).round() as i64;
    }
}
...
impl RewardStrategyDrawer for ExponentiallyIncreasingRewardStrategy {
    fn reward_points_to_drawer(&self, player: &mut Player, number_of_guessers: usize, players_already_guessed: usize) {
        player.score += (self.last_reward as f64 * ((1.0 - self.increase_per_position).powi(
            (number_of_guessers - players_already_guessed) as i32,
        )) as f64).round() as i64;
    }
}

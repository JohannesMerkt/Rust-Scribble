...
fn handle_message(
    ...
    if msg["msg_type"].eq("chat_message") {
        let _ = add_broadcast_message(msg, broadcast_queue);
    } else if msg["msg_type"].eq("guess") {
        println!("Guess received");
    }
}

fn read_broadcast_message(
    msg: serde_json::Value,
    broadcast_queue: &Arc<Mutex<VecDeque<serde_json::Value>>>,
) {
...
fn add_game_state_broadcast(
    ...
    match add_broadcast_message(json!(&game_state), broadcast_queue) {
        Ok(_) => Ok(()),
        Err(e) => Err(Box::new(e)),
    }
}
...
fn check_send_broadcast_messages(
    ...
    loop {
        let mut broadcast_msg: Option<serde_json::Value> = None;
        {
        ...
        match broadcast_msg {
            Some(msg) => match net_infos.try_lock() {
                ...
                Err(_) => {
                    read_broadcast_message(msg, &broadcast_queue);
                }
            },
            None => {}
        }
...
fn send_message(net_info: &mut NetworkInfo, msg: &serde_json::Value) -> io::Result<()> {
    send_tcp_message(
...
fn handle_client(
...
) {
    {
...

        {
            ...
            let _ = add_game_state_broadcast(&game_state, &broadcast_queue);
        }
    }
